...
mod tests {
    ...
    async fn test_pending_future_behavior() {
        ...
        // Create a custom body that returns Pending once. No Arc/Mutex needed because
        // `poll_frame` is called with `&mut self` and only the polled instance mutates state.
        #[derive(Clone)]
